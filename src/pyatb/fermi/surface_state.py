...
class Surface_State:
    ...
    def get_surface_state(
        ...
        for ik in self.__k_generator:
            ...
            converged_eps = 1e-10
            if kpoint_num:
                temp_spect_matrix_l, temp_spect_matrix_r = self.__tb_solver.get_surface_spectral_fun_by_Tmatrix(
                        self.__surface_direction,
                        self.__coupling_layers,
                        self.__calculate_layer,
                        self.__omega_num,
                        self.__domega,
                        self.__start_omega,
                        self.__eta,
                ...
                temp_spect_matrix_l = np.zeros([0, self.__calculate_layer, self.__omega_num], dtype=float)
                temp_spect_matrix_r = np.zeros([0, self.__calculate_layer, self.__omega_num], dtype=float)

            self.spect_matrix_l = COMM.reduce(temp_spect_matrix_l, root=0, op=op_gather_numpy)
            self.spect_matrix_r = COMM.reduce(temp_spect_matrix_r, root=0, op=op_gather_numpy)
    ...
    def print_data(self):
        ...
        with open(os.path.join(output_path, 'spectral_function_l.dat'), 'a+') as f:
            if self.first_print:
                f.write('# kpoint_number = %d, energy_points = %d, calculate_layer_nums = %d\n'%(self.__k_generator.total_kpoint_num, self.__omega_num, self.__calculate_layer))
            ...
            for ik in range(self.kvec_d.shape[0]):
                for i, ie in enumerate(energy_point):
                    f.write('%18.6f  '%(ie))
                    for i_layer in range(self.__calculate_layer):
                        f.write('%18.6f  '%(self.spect_matrix_l[ik, i_layer, i]))
                    f.write('\n')

        with open(os.path.join(output_path, 'spectral_function_r.dat'), 'a+') as f:
            if self.first_print:
                f.write('# kpoint_number = %d, energy_points = %d, calculate_layer_nums = %d\n'%(self.__k_generator.total_kpoint_num, self.__omega_num, self.__calculate_layer))
            ...
            for ik in range(self.kvec_d.shape[0]):
                for i, ie in enumerate(energy_point):
                    f.write('%18.6f  '%(ie))
                    for i_layer in range(self.__calculate_layer):
                        f.write('%18.6f  '%(self.spect_matrix_r[ik, i_layer, i]))
                    f.write('\n')
...
specfunc_l_file = os.path.join(work_path, 'spectral_function_l.dat')
specfunc_r_file = os.path.join(work_path, 'spectral_function_r.dat')
...
plot(specfunc_l_file, 'dos_surface_l.png')
plot(specfunc_r_file, 'dos_surface_r.png')
"""
            f.write(plot_script)
    ...
    def calculate_surface_state(
        ...
        **kwarg
        ...
        self.__calculate_layer = calculate_layer
