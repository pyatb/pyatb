...
class Bandunfolding_Spin_Texture:
    def __init__(
        self,
        tb: tb,
        **kwarg
        ...
        if tb.nspin != 4:
            raise ValueError('bandunfolding_spin_texture is meaningful only when nspin=4 !')
        ...
        output_path = os.path.join(OUTPUT_PATH, 'Bandunfolding_Spin_Texture')
        if RANK == 0:
    ...
    def get_bandunfolding_spin_texture(self, stru_file, ecut, band_range, m_matrix, **kwarg):
        COMM.Barrier()
        if RANK == 0:
            with open(RUNNING_LOG, 'a') as f:
                f.write('\nEnter the bandunfolding spin texture calculation module ==> \n')
                f.write('\nParameter setting : \n')
                f.write(' >> stru_file  : %s\n'%(stru_file))
                f.write(' >> ecut       : %f\n'%(ecut))
        ...
        if RANK == 0:
            ...
            self.spectral_weight_band = np.zeros([0, select_band_num], dtype=float)
            self.spectral_weight_sx = np.zeros([0, select_band_num], dtype=float)
            self.spectral_weight_sy = np.zeros([0, select_band_num], dtype=float)
            self.spectral_weight_sz = np.zeros([0, select_band_num], dtype=float)
        ...
        for ik in k_generator:
            ...
            if kpoint_num:
                P, P_sx, P_sy, P_sz, E = self.__tb_solver.get_bandunfolding_spin_texture(m_matrix, ik_process.k_direct_coor_local, ecut, min_bandindex, max_bandindex, self.nspin)
            else:
                P = np.zeros([0, select_band_num], dtype=float)
                P_sx = np.zeros([0, select_band_num], dtype=float)
                P_sy = np.zeros([0, select_band_num], dtype=float)
                P_sz = np.zeros([0, select_band_num], dtype=float)
                E = np.zeros([0, select_band_num], dtype=float)

            tem_P = COMM.reduce(P, root=0, op=op_gather_numpy)
            tem_P_sx = COMM.reduce(P_sx, root=0, op=op_gather_numpy)
            tem_P_sy = COMM.reduce(P_sy, root=0, op=op_gather_numpy)
            tem_P_sz = COMM.reduce(P_sz, root=0, op=op_gather_numpy)
            tem_E = COMM.reduce(E, root=0, op=op_gather_numpy)

            if RANK == 0:
                self.eig = tem_E
                self.spectral_weight_band = tem_P
                self.spectral_weight_sx = tem_P_sx
                self.spectral_weight_sy = tem_P_sy
                self.spectral_weight_sz = tem_P_sz
    ...
    def print_data(self):
        ...
        with open(os.path.join(output_path, 'spectral_weight_band.dat'), 'a') as f:
            if self.first_print:
            ...
            for i in range(self.eig.shape[0]):
                for ib in range(self.eig.shape[1]):
                    f.write('  %-15.6f %-10.6f\n'%(self.eig[i, ib], self.spectral_weight_band[i, ib]))

        with open(os.path.join(output_path, 'spectral_weight_spin_x.dat'), 'a') as f:
            if self.first_print:
                f.write('# kpoint_number = %d, band_number = %d\n'%(self.total_k_num, self.eig.shape[1])) 
                f.write('# %-15s %-10s\n'%('Energy', 'spectral_weight'))
            ...
            for i in range(self.eig.shape[0]):
                for ib in range(self.eig.shape[1]):
                    f.write('  %-15.6f %-10.6f\n'%(self.eig[i, ib], self.spectral_weight_sx[i, ib]))

        with open(os.path.join(output_path, 'spectral_weight_spin_y.dat'), 'a') as f:
            if self.first_print:
                f.write('# kpoint_number = %d, band_number = %d\n'%(self.total_k_num, self.eig.shape[1])) 
                f.write('# %-15s %-10s\n'%('Energy', 'spectral_weight'))
            ...
            for i in range(self.eig.shape[0]):
                for ib in range(self.eig.shape[1]):
                    f.write('  %-15.6f %-10.6f\n'%(self.eig[i, ib], self.spectral_weight_sy[i, ib]))

        with open(os.path.join(output_path, 'spectral_weight_spin_z.dat'), 'a') as f:
            if self.first_print:
                f.write('# kpoint_number = %d, band_number = %d\n'%(self.total_k_num, self.eig.shape[1])) 
                f.write('# %-15s %-10s\n'%('Energy', 'spectral_weight'))
            ...
            for i in range(self.eig.shape[0]):
                for ib in range(self.eig.shape[1]):
                    f.write('  %-15.6f %-10.6f\n'%(self.eig[i, ib], self.spectral_weight_sz[i, ib]))

        self.first_print = False

    def print_plot_script(self, fermi_energy):
        output_path = self.output_path
        sw_file = os.path.join(output_path, 'spectral_weight_band.dat')
...
""".format(fermi_energy=fermi_energy, sw_file=sw_file)
    ...
    def calculate_bandunfolding_spin_texture(self, kpoint_mode, **kwarg):
        COMM.Barrier()

        timer.start('bandunfolding_spin_texture', 'calculate bandunfolding_spin_texture')
        ...
        self.get_bandunfolding_spin_texture(**kwarg)

        timer.end('bandunfolding_spin_texture', 'calculate bandunfolding_spin_texture')
